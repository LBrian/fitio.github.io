...
var PageTransitions = (function () {
    ...
    function resetPage( $outpage, $inpage ) {
        ...
        $inpage.attr(
            'class', $inpage.data( 'originalClassList' )
            + ' pt-page-current'
        );
    }
            ...
            self._animateFull(
                {
                    ...
                    draw    : function ( progress ) {
                        ...
                        if ( self._getFit() === 'contain' ) {
                            ...
                            top = top > minTop ? (minTop + topWidth) - (progress * topWidth)
                                : (minTop - topWidth) + (progress * topWidth);
                            left = left > minLeft ? leftWidth + minLeft - (progress * leftWidth)
                                : (minLeft - leftWidth) + (progress * leftWidth);
                            bottom = bottom > minBottom ? bottomWidth + minBottom - (progress * bottomWidth)
                                : (minBottom - bottomWidth) + (progress * bottomWidth);
                            right = right > minRight ? rightWidth + minRight - (progress * rightWidth)
                                : (minRight - rightWidth) + (progress * rightWidth);
...
CanvasClip.prototype.closeFullCanvas  = function ( position, block ) {
    ...
    try {
        if ( canvas.getContext ) {
            ...
            var x1Start = 0, x1End = self._getPointFull( 0, 0 ),
                y1Start = 0, y1End = self._getPointFull( 0, 1 );
            var x2Start = canvas.width, x2End = self._getPointFull( 1, 0 ), x2Width = x2Start - x2End,
                y2Start = 0, y2End = self._getPointFull( 1, 1 );
            var x3Start = canvas.width, x3End = self._getPointFull( 2, 0 ), x3Width = x3Start - x3End,
                y3Start = canvas.height, y3End = self._getPointFull( 2, 1 ), y3Width = y3Start - y3End;
            var x4Start = 0, x4End = self._getPointFull( 3, 0 ),
                y4Start = canvas.height, y4End = self._getPointFull( 3, 1 ), y4Width = y4Start - y4End;
            //***************************************************************/
